...
def func_stimr():
    requests.post("http://120.78.149.223:9092/metrics/job/network_traffic",data=prometheus_client.generate_latest(REGISTRY))
    print("发送了一次网卡流量数据")

def dojob():
    schedule=BlockingScheduler()
    schedule.add_job(func_stimr,'interval',seconds=2,id='tt_job1')
    schedule.start()
